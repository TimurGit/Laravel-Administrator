...
class AdministratorServiceProvider extends ServiceProvider {
	...
	public function register()
	{
		...
		//the admin validator
		$this->app['admin_validator'] = $this->app->singleton('admin_validator',function($app)
		{
		...
		$this->app['admin_config_factory'] = $this->app->singleton('admin_config_factory',function($app)
		{
			return new ConfigFactory($app->make('admin_validator'), LValidator::make(array(), array()), config('administrator'));
		});

		$this->app['admin_field_factory'] = $this->app->singleton('admin_field_factory',function($app)
		{
			return new FieldFactory($app->make('admin_validator'), $app->make('itemconfig'), $app->make('db'));
		});

		$this->app['admin_datatable'] = $this->app->singleton('admin_datatable',function($app)
		{
		...
		$this->app['admin_column_factory'] = $this->app->singleton('admin_column_factory',function($app)
		{
			return new ColumnFactory($app->make('admin_validator'), $app->make('itemconfig'), $app->make('db'));
		});

		$this->app['admin_action_factory'] = $this->app->singleton('admin_action_factory',unction($app)
		{
			return new ActionFactory($app->make('admin_validator'), $app->make('itemconfig'), $app->make('db'));
		});

		$this->app['admin_menu'] = $this->app->singleton('admin_menu', function($app)
		{
